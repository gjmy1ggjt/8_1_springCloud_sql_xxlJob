 ...
 */
@Service
public class DashboardSupplierOrderDataBusiness implements DashboardDrawBusinessI {
    private static final String ECHART_NAME = "海外合作商走货量统计图";

    private Logger logger = LoggerFactory.getLogger(DashboardSupplierOrderDataBusiness.class);
    ...
    public DashboardVO drawDashboard(DashboardVO vo) {
        ...
        String postUrl = url + "/viewOrderData/getSupplierOrderCalData";
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", MediaType.APPLICATION_JSON_VALUE);
        HttpEntity<Map<String,Object>> entity = new HttpEntity<>(map, headers);
        //result 返回的json数据
        List<Map> result = simpleRestTemplate.postForObject(postUrl, entity, List.class);
        logger.debug("DashboardSupplierOrderDataBusiness-entity:{}", entity);
...
//        data.put("title", titleMap);
        ...
        logger.debug("=========> 海外合作商走货量统计图数据: {}", data.toString());
