 ...
 */
@Service
public class DashboardCustomerConsumeBusiness implements DashboardDrawBusinessI {

    private static final Logger logger = LoggerFactory.getLogger(DashboardCustomerConsumeBusiness.class);
    ...
    public DashboardVO drawDashboard(DashboardVO vo) {
        ...
        String postUrl = url + "/view/customerConsume";
        List<Map<String,Object>> result = restForStatistics(postUrl,map);
        DashboardUtil.sortData(result,"profit");
    ...
    public Map dataForJson(List<Map<String,Object>> result){
        ...
        for(Map item : result){
            if(index < 10) {
                if(item.get("simple_company_name") != null && !"".equals(item.get("simple_company_name"))) {
                    categorys.add("" +  item.get("simple_company_name"));
                }else{
                    categorys.add(""+item.get("customer_code"));
                }
                values.add(DataUtils.toBigDecimal(Float.parseFloat(item.get("profit")+"")).setScale(2, BigDecimal.ROUND_HALF_UP).floatValue());
                index++;
            }
        }
    ...
    public String[] getDrawBusinessName() {
        return new String[]{StatisticsDrawType.SALESMODULE, "客户消费金额排行榜TOP10"};
    }
}
